...
cdef np.ndarray __wrap_in_1d_int(object base, np.intp_t shape, INT_t * values):
        '''
        Wraps the given C array into NumPy array. The array keeps
        a reference to the Python object (`base`), which manages
        the underlying memory.
        '''
        cdef np.ndarray array = np.PyArray_SimpleNewFromData(1, &shape, np.NPY_INT32, values)
        Py_INCREF(base)
        array.base = <PyObject*> base
        return array
...
cdef class RankingSampler(object):
    cdef DOUBLE_t* S
    cdef DOUBLE_t* P
    cdef DOUBLE_t* C
    cdef INT_t*    D
    ...
    def __reduce__(self):
        ...
        '''
        return (RankingSampler, (__wrap_in_1d_double(self, self.L, self.S),
                                 self.G, self.random_state))

    def softmax_ranking(self, np.ndarray[INT_t, ndim=1] out=None):
        ''' 
